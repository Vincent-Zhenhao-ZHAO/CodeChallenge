...
#
# The logic behind doing this is ensuring that our duplicate arrays will always be in the same order,
# allowing us to filter them out by checking if they are already in the results list
#
...
def three_sum(arr):
    ...
    for idx in range(len(arr) - 2):
        ...
        first_num = sorted_arr[idx]
        second_num_idx = idx + 1
        third_num_idx = len(sorted_arr) - 1

        while second_num_idx < third_num_idx:

            the_sum = first_num + sorted_arr[second_num_idx] + sorted_arr[third_num_idx]

            if the_sum == 0:

                triplet = [first_num, sorted_arr[second_num_idx], sorted_arr[third_num_idx]]
